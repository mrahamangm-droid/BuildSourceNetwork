...
Branches & warehouses (Step 27): /dashboard/branches lets an owner or admin manage branches, each with one or more warehouses (limits 20 and 10). Stock can be recorded, filtered and reserved for an order per warehouse, and moved between warehouses as an ISSUE + RECEIPT pair in one Serializable transaction (only unreserved stock moves). A warehouse can be deleted only if it never had stock history, holds nothing and no order line points at it, because ledger rows cascade with the warehouse. No schema change: the Branch and Warehouse tables already existed.


Blog (Step 26): platform admins write articles at /admin/blog (drafts until published, first publication date kept) and readers see them at /blog and /blog/[slug] with Article structured data and sitemap entries. Articles use a small Markdown subset that is rendered to React elements, so author-typed HTML is never interpreted and only http(s) or site-relative links are allowed. Two evergreen guides (writing an RFQ, reading a quote) can be added as drafts from the admin page; they contain no statistics or claims about real companies and should be reviewed and edited before publishing.
