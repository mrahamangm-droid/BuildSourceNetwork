...
export default async function PlanRequestsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Plan requests" description="Confirm payment outside the platform, record the reference, then activate." />
      <div className="flex gap-3 text-sm">
        {(["PENDING", "APPROVED", "REJECTED"] as const).map((t) => (
          <Link key={t} href={`/admin/plans?tab=${t}`} className={status === t ? "font-semibold text-brand-700" : "text-slate-600"}>
      ...
      {rows.map((r) => (
        <Card key={r.id}>
          ...
          <p className="mt-2 text-sm">
            ...
            {r.plan.priceMonthlyCents ? ` ($${r.plan.priceMonthlyCents / 100}/month)` : ""} · current plan{" "}
            {r.org.subscription?.planCode ?? "none"} · sent {formatDate(r.createdAt)}
          </p>
          <p className="text-sm text-muted">Contact: {r.org.email ?? r.org.phone ?? "—"}</p>
          {r.note ? <p className="mt-1 text-sm">{r.note}</p> : null}
          {status === "PENDING" ? <PlanReviewForm requestId={r.id} /> : r.reviewNote ? <p className="mt-2 text-sm text-muted">Note: {r.reviewNote}</p> : null}
        </Card>
      ))}
    </div>
  );
