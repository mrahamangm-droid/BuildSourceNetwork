  ...
  return (
    <div>
      <div className="flex justify-between text-sm">
        <span>{label}</span>
        <span className="text-muted">{limit === null ? `${used} (unlimited)` : `${used} of ${limit}`}</span>
      </div>
      <div className="mt-1 h-2 overflow-hidden rounded-full bg-slate-100" role="progressbar" aria-valuenow={u.percent} aria-valuemin={0} aria-valuemax={100} aria-label={label}>
        <div
...
export default async function BillingPage() {
  ...
  return (
    <div className="max-w-3xl space-y-5">
      ...
      {b.limits.lapsed ? (
        <Alert tone="error">
          Your paid plan has ended, so Free limits apply. Nothing was removed; you just cannot add more than the
          Free limits until you renew.
        </Alert>
      ) : null}
      <Card>
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="text-lg font-semibold">{b.limits.planName}</h2>
          <Badge tone={b.limits.lapsed ? "red" : "green"}>{b.limits.lapsed ? "Ended" : "Active"}</Badge>
        </div>
        {b.subscription?.currentPeriodEnd && b.limits.planCode !== "FREE" ? (
          <p className="mt-1 text-sm text-muted">
            Paid until {formatDate(b.subscription.currentPeriodEnd)}
            {b.limits.daysLeft !== null && b.limits.daysLeft >= 0 ? ` (${b.limits.daysLeft} days left)` : ""}
          </p>
        ) : null}
      ...
      {pending ? (
        ...
        <PlanRequestForm
          plans={b.plans.filter((p) => p.code !== "FREE").map((p) => ({ code: p.code, name: p.name, priceMonthlyCents: p.priceMonthlyCents }))}
          currentCode={b.limits.planCode}
        />
      ) : (
        <Card className="text-sm text-muted">Ask an owner or admin of your company to change the plan.</Card>
      )}
      {b.requests.length ? (
        <Card>
          ...
          <ul className="mt-2 divide-y divide-line text-sm">
            {b.requests.map((r) => (
              <li key={r.id} className="flex flex-wrap items-center gap-2 py-2">
                ...
                <Badge tone={r.status === "APPROVED" ? "green" : r.status === "REJECTED" ? "red" : "amber"}>{r.status.toLowerCase()}</Badge>
