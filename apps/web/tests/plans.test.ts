...
import { effectiveLimits, nextPeriodEnd, requestablePlan, usageLevel, type PlanRow } from "../src/lib/plans";
...
describe("effectiveLimits", () => {
  it("gives Free limits with no subscription or an unknown plan", () => {
    ...
    expect(effectiveLimits({ planCode: "GONE", status: "ACTIVE", currentPeriodEnd: null }, plans, now).planCode).toBe("FREE");
  });
  it("applies a paid plan inside its period", () => {
    const e = effectiveLimits({ planCode: "STARTER", status: "ACTIVE", currentPeriodEnd: d("2026-10-20T00:00:00Z") }, plans, now);
    expect(e).toMatchObject({ planCode: "STARTER", productLimit: 200, lapsed: false, daysLeft: 10 });
  });
  it("keeps paid limits during the grace window, then falls back to Free", () => {
    const sub = { planCode: "STARTER", status: "ACTIVE", currentPeriodEnd: d("2026-10-05T00:00:00Z") };
  ...
  it("treats a non-active subscription as Free, and a paid plan with no end date as active", () => {
    expect(effectiveLimits({ planCode: "STARTER", status: "CANCELLED", currentPeriodEnd: d("2027-01-01T00:00:00Z") }, plans, now).lapsed).toBe(true);
    expect(effectiveLimits({ planCode: "SME", status: "ACTIVE", currentPeriodEnd: null }, plans, now)).toMatchObject({ planCode: "SME", productLimit: null, daysLeft: null });
  });
});
...
describe("nextPeriodEnd", () => {
  it("starts from today when lapsed or new, and extends a live period", () => {
    ...
    expect(nextPeriodEnd(d("2026-10-01T00:00:00Z"), now).toISOString()).toBe("2026-11-09T00:00:00.000Z");
    expect(nextPeriodEnd(d("2026-10-20T00:00:00Z"), now).toISOString()).toBe("2026-11-19T00:00:00.000Z");
  });
});
